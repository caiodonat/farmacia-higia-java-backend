...
public class CustomerController {
	...
	ResponseEntity<?> getAll() {
		try {
			List<Customer> customers = repo.selectAll();
	...
	@PostMapping("/login")
	ResponseEntity<?> loginCustomer(@RequestBody CustomerLogin reqCustomer) {
		try {

			Customer dbCustomer = repo.selectByEmail(reqCustomer.getEmail());
			if (dbCustomer == null) {

				res.put("message", "Cliente não encontrado:");
				res.put("errors", reqCustomer.getEmail());
				return ResponseEntity
						.status(400)
						.body(res);
			}
